...
//wichtige Config//
$self_host_url = 'http://localhost/Mediathek/liste.php'; //eigene vollständige URL (bspw.:http://localhost/Mediathek/liste.php)
                                       //ist nur notwenig für die automtische verarbeiten der Filmliste nach den Download ( liste.php?list_update__only_aufteilen=1 )
//$self_host_url = 'http://smart.ferienwohnung-sehestedt.de/liste.php'; //eigene vollständige URL (bspw.:http://localhost/Mediathek/liste.php)
if($self_host_url=='') die("Bitte die Datei liste.php bearbeiten: Erste Zeile: Config");

//Config Filmliste-Download
$filmlisteUrl       = "http://verteiler3.mediathekview.de/Filmliste-akt.xz";
...
//Config//
//Es gilt: 0=deaktiven; 1=aktiv //
//nun per Javascript $hideTrailer     = 1; //Wenn "Trailer" am Anfang im Titel bitte ausblenden (die Filmlist muss nach Wertänderung neu eingelesen werden liste.php?list_update__only_aufteilen=1)
//nun per Javascript $hideHoerfassung = 1; //Lässt sich unter Einstellungen von jeden  Benutzer auch selber festlegen; Ausblenden von Hörfassungen
$hideArte_fr     = 1; //Arte.fr ausblenden (0=deaktiviert) (der Cache muss nach Wertänderung neu erstellt werden: liste.php?list_update__only_aufteilen=1)
...
$timoutFilmlisteToOld = 0; //ab wann erscheint der Link: Filmliste neu laden werden (in Minuten) (wenn Shelllzugriff (=>$system_allow_exec_and_have_unxz=1)  bitte bei 0 belassen)
$filmlisten_autoUpdate = 1; //auch $filmlisteAutoReload_ifOlderThen=1 setzen //der Server lädt automatisch im Hintergrund eine neue Filmliste runter
$filmlisteAutoReload_ifOlderThen = 1; //empfohlen= auf 1 belassen //anzahl Minuten nachdem die Liste neu geladen werden darf (0=deaktiviert) (sollte man nur aktivieren, wenn das Lader+Verarbeiten der Liste auch nicht lange braucht)

$hideShorterThenList = array(1,2,3,5,10,20,25,55,85);//Minuten Auswahl möglichkeiten
$hideShorterThen = 0;
if(isset($_GET['hide_shorter_then']) && $_GET['hide_shorter_then']!='')$hideShorterThen = (int)$_GET['hide_shorter_then'];

//Config//
$PageTitle     = 'M.Liste';
$showSize      = 0; //0|1 Dateigröße anzeigen (nur für "Normal" Filmgröße)
$showMinDSLSpeed = 0; //0|1 Mindest notwendige DSL-Verbindung anzeigen (nur für "Normal" Filmgröße)
$welcomeText     = '<span style=\"\">Mediathek-liste in PHP, <br>&nbsp;geschrieben für TV-Browser & Hbb-TV.<br>Daten von MediathekView.de</span>';
...
Den Quellcode für diese Web-Anwendung finden Sie unter <a href="">TODO!!</a>
<h4>Verantwortlich für diesen Server ist:</h4>
&nbsp;&nbsp; Sören Jessen <br>
&nbsp;&nbsp; Harzhofer Weg 16<br>&nbsp;&nbsp; 24363 Holtsee<br>&nbsp;&nbsp; soerenj-os@web.de<br><br>
...
1) Die Filme werden von externen Seiten geladen (den Fernsehanstalten/bzw. der beauftragten IT-Dienstleistern).<br>
&nbsp;&nbsp;&nbsp;&nbsp;Bitte beachten Sie daher die <b>Datenschutz-Richtlinien von ARD,ZDF,ORF,ARTE,...</b>.<br>
&nbsp;&nbsp;&nbsp;&nbsp;Es wird lediglich die reine Filmdatei abgerufen.
2) Server-Standort: Frankreich 
3) Gespeichert werden von diesen Server die Verkehrsdaten für bis zu 4Tagen (zur Fehleranalyse oder evtl. anoynme Zugriffsstatistiken).<br>&nbsp;&nbsp;&nbsp;&nbsp;Eine längere Speicherung der Verkehrsdaten erfolgt nur in anonymer Form (IP-Adressen werden vollständig gelöscht).<br>
<br><br>
...
$cacheActive             = false;//true|false Client/Browser/Proxy-Cache aktvieren;  Beim testen besser ausschalten
...
$loaderAnimation              = 0;    //0|1|2 ladeKreis Animation (0=aus, 1=spinner.js, 2=drehendes utf8-Zeichen)
